...
sun3_long_instruction sun3_instr_cmpa_long = {
...
void sun3_cmpa_long(sun3 *vm)
{
	sun3_signed_long src, dest, ans;
	sun3_ea_mode_read_long(vm, &sun3_instr_cmpa_long);
	src= sun3_instr_cmpa_long.src;
	dest= sun3_address_regs_read_signed_long(&vm->address_regs, sun3_instr_move_reg(vm->pci));
	ans= dest-src;
