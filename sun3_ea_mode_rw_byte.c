...
void sun3_ea_mode_rw_byte(sun3 *vm, sun3_byte_instruction *i)
{
	switch(vm->ea_mode) {
	case sun3_instr_mode_DRD:
		if (i->legal_execute_modes&sun3_ea_mode_drd) {
			i->dest= sun3_data_regs_read_unsigned_byte(&vm->data_regs, vm->reg_num);
			i->execute(i);
			sun3_data_regs_write_unsigned_byte(&vm->data_regs, vm->reg_num, i->ans);
	...
	case sun3_instr_mode_ARD:
		if (i->legal_execute_modes&sun3_ea_mode_ard) {
			i->dest= sun3_address_regs_read_unsigned_byte(&vm->address_regs, vm->reg_num);
			i->execute(i);
			sun3_address_regs_write_unsigned_byte(&vm->address_regs, vm->reg_num, i->ans);
	...
	case sun3_instr_mode_ARI:
		if (i->legal_execute_modes&sun3_ea_mode_ari) {
			...
			i->dest= sun3_memory_read_unsigned_byte(&vm->memory, addr);
			i->execute(i);
			sun3_memory_write_byte(&vm->memory, addr, i->ans);
	...
	case sun3_instr_mode_ARIPI:
		if (i->legal_execute_modes&sun3_ea_mode_aripi) {
			...
			i->execute(i);
			sun3_memory_write_byte(&vm->memory, addr, i->ans);
			sun3_address_regs_write_unsigned_long(&vm->address_regs, vm->reg_num, addr+sizeof(sun3_unsigned_byte));
	...
	case sun3_instr_mode_ARIPD:
		if (i->legal_execute_modes&sun3_ea_mode_aripd) {
			sun3_memory_address addr= sun3_address_regs_read_unsigned_long(&vm->address_regs, vm->reg_num)-sizeof(sun3_unsigned_byte);
	...
	case sun3_instr_mode_EXTD:
		...
		case sun3_instr_mode_ID:
			if (i->legal_execute_modes&sun3_ea_mode_id) {
				i->dest= sun3_memory_read_signed_word(&vm->memory, vm->pc);
				i->execute(i);
