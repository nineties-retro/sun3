...
static sun3_word_instruction sun3_instr_move_word = {
...
void sun3_move_word(sun3 *vm) 
{
	sun3_ea_mode_read_word(vm, &sun3_instr_move_word);

	switch (sun3_instr_move_mode(vm->pci)) {
	case sun3_instr_mode_DRD:
	{
		unsigned int reg = sun3_instr_move_reg(vm->pci);
		sun3_trace_instr(vm, "move.w %"sun3_unsigned_long_fmtx", d%u", sun3_instr_move_word.src, reg);
		sun3_data_regs_write_signed_word(&vm->data_regs, reg, sun3_instr_move_word.src);
		break;
	}
	case sun3_instr_mode_ARD:
	{
		unsigned int reg = sun3_instr_move_reg(vm->pci);
		sun3_trace_instr(vm, "move.w %"sun3_unsigned_word_fmtx", a%u", sun3_instr_move_word.src, reg);
		sun3_address_regs_write_signed_word(&vm->address_regs, reg, sun3_instr_move_word.src);
	...
	case sun3_instr_mode_ARI:
	{
		...
		sun3_trace_instr(vm, "move.w %"sun3_unsigned_word_fmtx" (a%u) [%"sun3_memory_address_fmtx"]", sun3_instr_move_word.src, reg, addr);
		sun3_memory_write_word(&vm->memory, addr, sun3_instr_move_word.src);
		break;
	}
	case sun3_instr_mode_ARIPI:
	{
		unsigned int reg = sun3_instr_move_reg(vm->pci);
		sun3_memory_address addr = sun3_address_regs_read_unsigned_long(&vm->address_regs, reg);
		sun3_address_regs_write_unsigned_long(&vm->address_regs, reg, addr+sizeof(sun3_unsigned_word));
		sun3_memory_write_word(&vm->memory, addr, sun3_instr_move_word.src);
		break;
	}
	case sun3_instr_mode_ARIPD:
	{
		unsigned int reg = sun3_instr_move_reg(vm->pci);
		sun3_memory_address addr = sun3_address_regs_read_unsigned_long(&vm->address_regs, reg);
		addr -= sizeof(sun3_unsigned_word);
	...
	sun3_sr_set_n(vm->sr, ((sun3_signed_word)sun3_instr_move_word.src)<0);
	sun3_sr_set_z(vm->sr, sun3_instr_move_word.src==0);
