...
sun3_byte_instruction sun3_instr_sub_reg_byte =
{ 0
, 0
, 0
...
void sun3_sub_reg_byte(sun3 *vm)
{
	...
	sun3_ea_mode_read_byte(vm, &sun3_instr_sub_reg_byte);
	src= sun3_instr_sub_reg_byte.src;
	ans= dest-src;
...
static void sun3_instr_sub_ea_byte_execute(sun3_byte_instruction * sub)
{
...
static void sun3_instr_sub_ea_byte_set_flags(sun3_byte_instruction * sub, sun3_sr * sr)
{
	sun3_sr_set_v(*sr, ( ( (!sun3_unsigned_byte_sign(sub->src))
			     &  sun3_unsigned_byte_sign(sub->dest)
			     & (!sun3_unsigned_byte_sign(sub->ans))
			     )
			   | (  sun3_unsigned_byte_sign(sub->src)
			     & (!sun3_unsigned_byte_sign(sub->dest))
			     &  sun3_unsigned_byte_sign(sub->ans)
			     )
			   ));
	sun3_sr_set_c(*sr, ( (  sun3_unsigned_byte_sign(sub->src)
			     & (!sun3_unsigned_byte_sign(sub->dest))
			     )
			   | (  sun3_unsigned_byte_sign(sub->ans)
			     & (!sun3_unsigned_byte_sign(sub->dest))
	...
	sun3_sr_set_n(*sr, ((sun3_signed_byte)sub->ans)<0);
}


static sun3_byte_instruction sun3_instr_sub_ea_byte = {
	...
	.execute = sun3_instr_sub_ea_byte_execute,
	...
	.set_flags = sun3_instr_sub_ea_byte_set_flags
};


void sun3_sub_ea_byte(sun3 *vm)
{
	unsigned int src_reg = sun3_instr_move_reg(vm->pci);
	sun3_instr_sub_ea_byte.src = sun3_data_regs_read_unsigned_byte(&vm->data_regs, src_reg);
	sun3_ea_mode_rw_byte(vm, &sun3_instr_sub_ea_byte);
}
