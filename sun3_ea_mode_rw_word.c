...
void sun3_ea_mode_rw_word(sun3 *vm, sun3_word_instruction *i)
{
	switch(vm->ea_mode) {
	case sun3_instr_mode_DRD:
		if (i->legal_execute_modes&sun3_ea_mode_drd) {
			i->dest= sun3_data_regs_read_unsigned_word(&vm->data_regs, vm->reg_num);
			i->execute(i);
			sun3_data_regs_write_unsigned_word(&vm->data_regs, vm->reg_num, i->ans);
	...
	case sun3_instr_mode_ARD:
		if (i->legal_execute_modes&sun3_ea_mode_ard) {
			i->dest= sun3_address_regs_read_unsigned_word(&vm->address_regs, vm->reg_num);
			i->execute(i);
			sun3_address_regs_write_unsigned_word(&vm->address_regs, vm->reg_num, i->ans);
	...
	case sun3_instr_mode_ARI:
		if (i->legal_execute_modes&sun3_ea_mode_ari) {
			...
			i->dest= sun3_memory_read_unsigned_word(&vm->memory, addr);
			i->execute(i);
			sun3_memory_write_word(&vm->memory, addr, i->ans);
