...
sun3_word_instruction sun3_instr_cmpa_word = {
...
void sun3_cmpa_word(sun3 *vm)
{
	sun3_signed_word src, dest, ans;
	sun3_ea_mode_read_word(vm, &sun3_instr_cmpa_word);
	src= sun3_instr_cmpa_word.src;
	dest= sun3_address_regs_read_signed_word(&vm->address_regs, sun3_instr_move_reg(vm->pci));
	ans= dest-src;
