...
sun3_word_instruction sun3_instr_sub_reg_word = {
...
void sun3_sub_reg_word(sun3 *vm)
{
	...
	sun3_ea_mode_read_word(vm, &sun3_instr_sub_reg_word);
	src= sun3_instr_sub_reg_word.src;
	ans= dest-src;
...
static void sun3_instr_sub_ea_word_execute(sun3_word_instruction * sub)
{
...
static void sun3_instr_sub_ea_word_set_flags(sun3_word_instruction * sub, sun3_sr * sr)
{
	sun3_sr_set_v(*sr, ( ( (!sun3_unsigned_word_sign(sub->src))
			     &  sun3_unsigned_word_sign(sub->dest)
			     & (!sun3_unsigned_word_sign(sub->ans))
			     )
			   | (  sun3_unsigned_word_sign(sub->src)
			     & (!sun3_unsigned_word_sign(sub->dest))
			     &  sun3_unsigned_word_sign(sub->ans)
			     )
			   ));
	sun3_sr_set_c(*sr, ( (  sun3_unsigned_word_sign(sub->src)
			     & (!sun3_unsigned_word_sign(sub->dest))
			     )
			   | (  sun3_unsigned_word_sign(sub->ans)
			     & (!sun3_unsigned_word_sign(sub->dest))
	...
	sun3_sr_set_n(*sr, ((sun3_signed_word)sub->ans)<0);
}


static sun3_word_instruction sun3_instr_sub_ea_word = {
	...
	.execute = sun3_instr_sub_ea_word_execute,
	...
	.set_flags = sun3_instr_sub_ea_word_set_flags
};


void sun3_sub_ea_word(sun3 *vm)
{
	unsigned int src_reg = sun3_instr_move_reg(vm->pci);
	sun3_instr_sub_ea_word.src = sun3_data_regs_read_unsigned_word(&vm->data_regs, src_reg);
	sun3_ea_mode_rw_word(vm, &sun3_instr_sub_ea_word);
}
